...
All synthetic state is inspectable in the browser and saved only in local storage when available. Two browser-local storage classes exist and are not equivalent:

- **Client uploads (PBC responses, workpaper evidence):** metadata plus a SHA-256 digest only; the original bytes stay in-session (source is labelled `Local In-Session`) and must be reselected after a reload.
