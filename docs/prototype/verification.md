...
| 2026-09-24 | `a114e7b58af85d615d48f01a030e22be92922069` — VP-046 revision-bound consolidated output package and independent review | `npm run lint`; `npm run test:unit`; `npm run test:e2e`; `git diff --check`; Wrangler Pages production deployment + live asset verification | PASS — 170/170 unit; 73/73 E2E (5 static + 68 Chrome); production build passed | AT-42/AT-43/AT-45 Chrome exercises supported group calculation, pinned component review, rate/elimination inputs, then prepares a watermarked JSON artifact and independently approves it as a separate partner. The output contains only `ENG-26001` and `ENG-26002`, exact package/source/rate/elimination references and reconciled totals; its persisted IndexedDB bytes match the recorded SHA-256. Store checks bind approval to the current input fingerprint, reject stale review and disallow direct package-history edits. Existing `steaudit-prototype` Pages project deployment `ebcce409-2eb9-4bec-a84e-9ea41a62b22b` is Production on `production`, source `a114e7b`. The [immutable release](https://ebcce409.steaudit-prototype.pages.dev), stable Pages alias and [custom domain](https://prototype.steaudit.com) all return HTTP 200 and serve the same local-build JavaScript `assets/index-BnOkJzCB.js` (2,363,063 bytes; SHA-256 `029b34b375fe92ecb00c0c51e6be6f66924d1842dd1556ef5cd18503a048e777`) and CSS `assets/index-W5oEz7Fx.css` (43,700 bytes; SHA-256 `4fd2006049185501c7d65a4f9c16ef6e3b89d9ac634856733c4d842c496f550a`). Immutable/stable Pages HTML SHA-256 `40c31665d0c0755e43b8b55bb4d1bc336007224c02fb73c7b58ecb288cc31430` matches local `dist/index.html`; the custom domain injects Cloudflare's own analytics beacon, so its HTML hash differs while the application JS/CSS bytes match exactly. VP-046 remains Partial pending the full same-currency/FX value and export-lineage matrix.
