...
| 2026-09-24 | VP-042 atomic artifact persistence | `npm run lint`, `npm run test:unit`, `npm run test:e2e`, focused AT-41/42/48 Chrome | PASS — 149/149 unit, 67/67 E2E; focused package journey PASS | Chrome injects a synchronous failure on the second IndexedDB artifact write. The transaction aborts, the surfaced error is retained, the artifact count is unchanged, and package revision 2 is not saved. The same journey then restores IndexedDB and successfully persists the complete XLSX/DOCX/PDF revision. Replacement-revision management approval remains unverified. |
