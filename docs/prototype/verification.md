...
|---|---|---|---|---|
| 2026-09-23 | current working tree | `npm run build` | PASS | TypeScript check and Vite production build passed; existing large-main-chunk advisory remains (2.13 MB, 596.97 kB gzip). |
| 2026-09-23 | current working tree | `npm run test:unit` | PASS — 131/131 | Includes schema v0–v18 migration, VP-052 workpaper provenance/reassignment, VP-053 evidence-link and adequacy history, VP-054 finding amounts/source guards/disposition and release gates, mapping approvals, sampling, packages and finance. |
| 2026-09-23 | current working tree | `npm run test:e2e` | PASS — 56/56 | Five static checks plus 51 serial Chrome journeys. VP-052 verifies fresh template work and exact submitted-revision clearance; VP-053 verifies reasoned evidence unlink and stale procedure; VP-054 verifies a sourced qualitative finding, contextual materiality note and durable rationale after reload. 37 modules remain Partial. |
