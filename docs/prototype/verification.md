...
| 2026-09-24 | VP-055 scoped review queue and assignment history / release `8a95a8d` | `npm run lint`, `npm run test:unit`, `npm run test:e2e` | PASS — 142/142 unit, 60/60 E2E | E2E includes five static checks and 55 Chrome journeys. AT-55 verifies the permitted cross-engagement queue, row context, correct engagement response target and selected-engagement filter. Unit checks reject ineligible review assignees and require reasoned reassignment while retaining person/actor/time history. Commit `8a95a8d` is on `main` and deployed to the existing `steaudit-prototype` Production / `production` Pages project as `0aa48f1d-f347-479d-a72d-0e637cb9244d`. The release URL and `https://prototype.steaudit.com` both returned HTTP 200 and served `assets/index-BZ0UFl8d.js` (2,205,538 bytes; SHA-256 `339ff45e785740a497ae33b35ee22ff4e4e3e5e5d9309e2c27c50730d7bf2f21`). The hostname first returned the previous HTML bundle, then served the deployment after a no-cache request; its fetched JS bytes match the release and local build. Overall acceptance remains Partial; non-workpaper subjects, advanced queue filters and exports remain open. |
