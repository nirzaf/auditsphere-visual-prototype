...
|---|---|---|---|---|
| 2026-09-23 | current working tree | `npm run build` | PASS | TypeScript check and Vite production build passed; existing large-main-chunk advisory remains (2.097 MB, 590.63 kB gzip). |
| 2026-09-23 | current working tree | `npm run build` | PASS | TypeScript check and Vite production build passed; existing large-main-chunk advisory remains (2.1 MB, about 594 kB gzip). |
| 2026-09-23 | current working tree | `npm run test:unit` | PASS — 128/128 | Includes schema v0–v17 migration, VP-052 template provenance/reassignment, VP-053 scoped link/unlink/adequacy history and stale dependent work, account mapping approvals, sample frame guards, package artifacts, finance, and guard regressions. |
| 2026-09-23 | current working tree | `npm run test:e2e` | PASS — 55/55 | Five static checks plus 50 serial Chrome journeys. VP-052 verifies template-based fresh work, a genuine sample workbook, versioned edits, pinned evidence, submission and exact assigned-reviewer clearance. VP-053 verifies reasoned unlink history and stale dependent fieldwork. All existing journeys pass; 37 modules remain Partial as listed in `module-coverage.md`. |
