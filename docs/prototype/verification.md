...
|---|---|---|---|---|
| 2026-09-23 | current working tree | `npm run test:unit` | PASS — 113/113 | Schema v11 migrations restore sample scope, selection, and source metadata; parser checks quoted CSV, genuine XLSX, duplicates and formulas; store checks require selection and preserve tested predecessors on replacement. |
| 2026-09-23 | current working tree | `npm run test:e2e` | PASS — 40/40 | Five static checks and 35 actual Chrome checks. VP-051 imports CSV rows, computes totals, preserves exception history, rejects duplicate replacement, and verifies digest/history after reload. |
| 2026-09-23 | current working tree | `npm run build` | PASS | TypeScript clean; Vite emitted the app. Main JS bundle is about 2.03 MB (575.17 kB gzip), above Vite's 500 kB advisory threshold. |
...
current checks cover schema migrations 0–10 and browser storage conflict/quota
