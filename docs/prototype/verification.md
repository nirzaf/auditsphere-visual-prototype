...
| 2026-09-24 | VP-055 finding review subjects / release `f84463a` | `npm run lint`, `npm run test:unit`, `npm run test:e2e` | PASS — 143/143 unit, 61/61 E2E | E2E includes five static checks and 56 Chrome journeys. Finding review notes pin to the finding revision; changing its disposition reopens the note, keeps the old response, and does not change the finding disposition from clearing. Commit `f84463a` is on `main` and deployed to the existing `steaudit-prototype` Production / `production` Pages project as `229da141-706f-404c-a31e-40279b3eff61`. The release URL and `https://prototype.steaudit.com` both returned HTTP 200 and served `assets/index-Cjm9wBDK.js` (2,207,256 bytes; SHA-256 `afdae68d5972c75ba7f20c2804a0a8f33072f187d1004aa8fae4b13b81c77719`). The custom-host asset hash matches the release and local build. VP-055 remains Partial for other non-workpaper subjects, more filters and exports. |
