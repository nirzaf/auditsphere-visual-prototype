...
| 2026-09-24 | VP-055 scoped review queue / release `93b2a3e` | `npm run lint`, `npm run test:unit`, `npm run test:e2e` | PASS — 141/141 unit, 60/60 E2E | E2E includes five static checks and 55 Chrome journeys. New AT-55 journey verifies a note from another engagement appears in the permitted cross-engagement queue and is excluded from the selected-engagement filter. Commit `93b2a3e` is on `main` and deployed to the existing `steaudit-prototype` Production / `production` Pages project as `276fbe67-6f4a-4c33-b531-f9699fdf2bd3`. Its release URL and `https://prototype.steaudit.com` both returned HTTP 200 and served `assets/index-vUYANHJD.js` (2,202,815 bytes; SHA-256 `f2adc116d93a44b24653bef1a1b80e8c985383b8282b36ac807a7ddb6c1f83d8`). The bundle contains the queue scope filter and labels. Overall acceptance remains Partial; assignment eligibility/history, non-workpaper subjects and additional filters/exports remain open. |
