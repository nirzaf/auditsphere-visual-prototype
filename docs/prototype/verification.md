...
acceptance assertions. Commit `3831163` is deployed as Pages release
`89b2434a-aa9b-46f3-b4dd-4cb83290b976` in the existing `steaudit-prototype`
...
| 2026-09-23 | current working tree | `npm run test:unit` | PASS — 101/101, 22 suites | AT-05/06 store checks cover custom field type validation, disable-with-value history retention, contact non-authorization, primary contact uniqueness and relationship-group/access separation. |
| 2026-09-23 | current working tree | `npm run test:e2e` | PASS — 21/21 | Five static checks and sixteen actual Chrome checks. AT-05/06 creates a client, primary contact, choice definition/value and relationship group, verifies two-client group membership without grant changes, and opens all 12 client workspace tabs without cross-client engagement leakage. |
