...
| 2026-09-24 | a8463c5 | VP-036 GL import, source revisions and opening-to-closing tie-out | PASS — `npm run lint`, `npm run test:unit` 167/167, and `npm run test:e2e` 71/71 (5 static + 66 Chrome). AT-36 imports a period-bound engagement GL source, checks the digest/revision, reconciles each TB account using explicit opening balances and movements, verifies filtered transaction drill-down, then reloads and confirms the source and complete tie-out persist. Unit tests cover real XLSX parsing, opening-only rows, unbalanced journals, duplicate line keys, wrong periods/currencies/dates, atomic rejection, immutable replacement history, unchanged TB rows, reconciliation staleness and fixture integrity. VP-036 remains Partial: user-configurable header mapping and broader partial-batch/output-rework matrices remain open. |
