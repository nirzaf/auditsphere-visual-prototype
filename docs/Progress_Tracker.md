...
#### Pending actions
The distinction between implementation, evidence and scope reconciliation is intentional.
- [ ] **VP-017-E01 — Verification/evidence pending:** AT-15 proves an invalid HTTP SharePoint URL is rejected with saved configuration preserved; RR35 rejects blank tenant IDs, invalid roots/mailboxes and unknown permitted personas atomically. AT-15 changes to a mismatched synthetic tenant, records `missing-resource` and `Simulated error` with `liveConnected=false`, then explicitly retries successfully. Full verification passes 196/196 unit and 83/83 E2E. Wizard start/back/cancel/review-summary, wider wrong-tenant/site combinations and outage-recovery matrix remain open.
...
| VP-017-E01 | [VP-017](#vp-017) | P2 | Complete the setup start/back/cancel/review-summary path and every invalid tenant/resource/person selection while proving liveConnected remains false. | Partial evidence: focused AT-15 verifies mismatched tenant → missing-resource → explicit recovery; full navigation/review and invalid-resource matrix remain open |
