...
| Unit checks passing in this review | 231 / 231 | Fresh full local run on 2026-09-26 at `e6a6edb`; adds the review-guide regression coverage (search authorization `canReadSearchRecord`, package-context selector and related guards) to the prior 216. |
| E2E checks passing in this review | 105 / 105 | Fresh full run 2026-09-26: 5 static + 100 Chrome; includes the new F01–F05 browser regressions (scoped search projection, draft-safe destructive reset, truthful context header, unique navigation ID containment, budget/template draft guards). |
...
| Open acceptance actions in Sections 9.3–9.4 | 41 | Programmatic recount of the detailed tables on 2026-09-26 (per the review guide §8): Section 9.3 has 37 open verification rows — 29 Partial, 5 Pending and 3 verified-subcase-only rows with explicit follow-up — and Section 9.4 has 4 open scope rows (3 Partial, 1 Open). 37 + 4 = 41 action rows; the previous header figure of 47 was stale and is corrected from the underlying records rather than by closing actions. Completed queue actions are not story sign-offs. 48 stories remain Partial. Counts cover queue actions, not discovered GitHub issues. |
...
#### Pending actions
The distinction between implementation, evidence and scope reconciliation is intentional.
- [ ] **VP-003-E01 — Partial verification evidence:** Shared semantics, focus containment, Escape, Enter submission and focus restoration pass for selected client/New Job dialogs. Chrome verifies client-profile and New Job Cancel/backdrop dismissal discards drafts and restores focus (AT-53, 2026-09-25). Client edit focus restoration also passes (AT-05/AT-06). AT-26 verifies simulated-compose Cancel/backdrop dismissal and AT-14 verifies Internal Note Cancel/backdrop dismissal, with no persisted draft and focus restored. Jobs & Tasks edit-job Cancel/backdrop dismissal now leaves stored title unchanged and returns focus to its opener; edit-task close/backdrop and reassignment backdrop paths close cleanly. On 2026-09-26, Global Search receives an explicit accessible name; focused Chrome verifies dialog semantics, tab wrap/focus containment, Escape/backdrop close and no persisted search draft. The Scenario Chooser's titled modal also passes focused Chrome Escape/backdrop dismissal with no scenario load, route change, or persisted-state mutation. Dialog-specific save/cancel/dismissal paths for the rest of the active-modal inventory and criterion sign-off remain open.
- [ ] **VP-003-E02 — Partial implementation/evidence:** The keyed `UnsavedFormGuard` registry and client-context interception are implemented. M365 setup, Client 360 Add Contact, proposal, invoice, cash-flow, package disclosure, and consolidation output forms have Chrome Stay/Save/Discard cases. The consolidation output journey also verifies Stay/Discard on perimeter and elimination drafts with unchanged saved group state. A dirty M365 setup form now covers a direct hash change to a route denied to the administrator: Stay restores the accepted route/hash, while Discard applies authorization and redirects to Overview. M365 setup cross-client search and AT-57 output draft guards are covered. Accounting Workbench adjustment drafts compare against modal-open defaults: opening/leaving untouched creates no prompt or journal, editing prompts and Discard drops the draft. Communications email and call/meeting note modals register separate guards; Chrome verifies an edited simulated email survives Stay and is not recorded when discarded. Time Tracking now guards changed entries/corrections and return reasons; Chrome verifies clean defaults, Stay, Discard without persistence, and Save before navigation. Receivables receipt and allocation drafts now register guards; Chrome verifies client-switch Stay/Discard, navigation Discard without ledger mutation, and receipt Cancel/backdrop focus restoration. Documents registration and Workpaper replacement drafts now guard route navigation; Chrome verifies Stay retention and Discard without adding metadata/revision. Review Desk raise and response drafts each guard route navigation; Chrome verifies Stay retention and Discard without creating a note or changing the saved response. Release Completion guards local dispatch note/recipient metadata and amendment reason drafts; AT-41 verifies Stay/Discard and no unintended issue/reopen. Jobs & Tasks new job, new task, internal note, job edit, task edit, and reassignment drafts now guard route navigation; Chrome verifies Stay preservation and Discard without record creation or job/task/assignee mutation, plus Save of a new job before leaving. Full E2E 92/92 (5 static + 87 Chrome), 202/202 unit, lint/build/diff checks pass. Registry coverage also includes Audit Risks & Programs, Accounting Setup/TB staging, Audit Planning, packages, consolidation perimeter/eliminations/reviews/FX, and billing. Broader role/client/engagement/direct-target matrices and remaining modal families remain open.
- [ ] **VP-003-R03 — Partial requirement/scope reconciliation:** Reuse the keyed `UnsavedFormGuard` for missing form cases; Chrome now covers proposal, invoice, cash-flow/disclosure, consolidation output/perimeter/elimination drafts and several other active forms. The dirty direct-hash journey confirms Stay restores its previously accepted URL and Discard redirects a denied destination; broader form and role/client/engagement matrices remain open. Do not add another form framework.
#### Original user story and dependencies
**Target modules:** Cross-cutting foundation  
...
#### Pending actions
The distinction between implementation, evidence and scope reconciliation is intentional.
- [ ] **VP-017-E01 — Verification/evidence pending:** AT-15 proves an invalid HTTP SharePoint URL is rejected with saved configuration preserved; RR35 rejects blank/overlength tenant ID and domain, credentialed or queried SharePoint URLs, blank/overlength library names, relative/traversing/escaped roots, malformed mailboxes, missing/duplicate/inactive permitted identities and unknown roles atomically. AT-15 holds the SharePoint site/library/root fixed while changing the synthetic tenant, records `missing-resource` and `Simulated error` with `liveConnected=false`, then retries; stale identity still prevents overall readiness. It separately changes only the folder root, proves prior results stale, simulates access denied and retries. Full verification passes 197/197 unit and 83/83 E2E. AT-15 verifies explicit Start and Skip, Back/Continue, Cancel draft discard, permission-aware review/save and reload. Wider wrong-tenant/site/person combinations and outage-recovery matrix remain open.
...
- [ ] **VP-036-E03 — Partial verification evidence:** Store checks prove a second GL source revision preserves TB rows and prior GL revisions while staling reconciliations and invalidating release generation. AT-36 Chrome now starts with a frozen candidate and current reconciliations, imports the GL source, verifies original TB rows are byte-equivalent, each seeded reconciliation is marked Stale and displayed in the Reconciliations tab, the candidate is cleared, and generation advances. Focused AT-36 passed 1/1, 2026-09-25. Reviewed package-history rework and post-replacement reconciliation review cycle remain open.
#### Original user story and dependencies
**Target modules:** 21  
...
**Next evidence update:** Base commit `c5509784cdc8c3240db9105961be43755a0c6cc0` plus current E2E working-tree delta; fixture: deterministic ENG-26001 / AJ-01, TB source v1→v2 and evidence references `TB-IMPORT-REV-1/2`; AC02/03 and E02 → `tests/e2e/app.test.ts::VP-038-E01/E02` and `tests/unit/lifecycleGaps.test.ts::trial-balance replacement stales journal reflection`; lint/build, 226/226 unit, full serial E2E 99/99, 2026-09-26. Reviewer/date: **pending human sign-off**; VP-038 story remains Partial.
<a id="vp-039"></a>
...
This queue contains **47 open planning actions** attached to the 48 Partial stories: 40 evidence/verification actions (28 Partial, 10 Pending, and 2 verified-subcase rows with explicit follow-up still open) and 7 requirement/scope reconciliation actions. The counts are updated on 2026-09-26 after VP-038-E02 completion. Current branch issues, PRs, assignees and due dates were not queried; link them before using this as a team execution board. No action is permission to merge or deploy.
...
| VP-017-E01 | [VP-017](#vp-017) | P2 | Complete the setup start/back/cancel/review-summary path and every invalid tenant/resource/person selection while proving liveConnected remains false. | Partial evidence: AT-15 verifies Start/Skip, Back/Continue, cancel rollback, permission-aware review/save, mismatched tenant with unchanged resource → missing-resource → explicit recovery; root-only change → stale results/access-denied → recovery; full invalid selection/resource/outage combinations remain open; lint/build, 197/197 unit, focused RR35 + AT-15, full E2E 83/83 |
...
| VP-036-E03 | [VP-036](#vp-036) | P1 | Prove GL replacement stales affected reconciliation/package outputs without mutating historical source rows. | Partial — unit verifies second source revision retains prior GL revision and TB rows while staling reconciliation history and invalidating generation; AT-36 focused Chrome now verifies unchanged TB rows, stale reconciliation statuses in storage and UI, frozen release-candidate clearing and generation advance after source import. Reviewed package-history rework and post-replacement review cycle remain open, 2026-09-25 |
| VP-038-E01 | [VP-038](#vp-038) | P1 | Complete rejected/partial/unknown/reflected source permutations, journal amendments and linkage to evidence/workpaper/finding revisions. | Partial / AT-38 covers evidence-required reflection, partial/unknown exclusion, history, amendment and reapproval; VP-038-E01/E02 Chrome proves the current-source replacement/re-confirmation path. Remaining evidence/workpaper/finding linkage and edge permutations stay open. |
